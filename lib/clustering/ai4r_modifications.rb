  ...
  module GeneticAlgorithm
    ...
    class StaticChromosome < Chromosome
      ...
      def initialize(data, options)
        ...
        @consumer_ids = options[:consumer_ids]
      end
...
#        puts "reproduce, #{point1}, #{point2}"
      ...
      def self.seed(options)
        # puts "#{options}"
        data_size = options[:consumer_ids].length
        kappa = options[:kappa]
        ...
        options[:penalties_before] = options[:consumer_ids].map do |pid|
