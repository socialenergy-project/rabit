...
class DrEventsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create dr_event as admin' do
    ...
    assert_difference('DrEvent.count') do
      post dr_events_url, params: { dr_event: { 'interval_id' => @dr_event.interval_id, 'name' => @dr_event.name, 'price' => @dr_event.price, 'starttime' => @dr_event.starttime, 'state' => @dr_event.state, 'dr_type' => @dr_event.dr_type } }
    end
  ...
  test 'should update dr_event as admin' do
    sign_in_as_admin
    patch dr_event_url(@dr_event), params: { dr_event: { 'interval_id' => @dr_event.interval_id, 'name' => @dr_event.name, 'price' => @dr_event.price, 'starttime' => @dr_event.starttime, 'state' => @dr_event.state, 'dr_type' => @dr_event.dr_type } }
  ...
  #     @dr_event.dr_targets.map(&:dr_actions).map(&:destroy_all)
  #     delete dr_event_url(@dr_event)
  #   end
  #   assert_redirected_to dr_events_url
  # end

  test 'should NOT destroy dr_event with actions as admin' do
    sign_in_as_admin
    assert_no_difference('DrEvent.count', -1) do
      delete dr_event_url(@dr_event)
    end
    assert_redirected_to dr_events_url
  end
